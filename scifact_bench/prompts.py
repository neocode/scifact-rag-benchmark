"""All prompts used by the benchmark.  ``export_prompts()`` writes them verbatim to prompts.md."""
from __future__ import annotations

from typing import Dict, List

# --------------------------------------------------------------------------- generation
GENERATION_2CLASS = """Task: scientific claim verification using ONLY the evidence sentences provided.
...
GENERATION_3CLASS = """Task: scientific claim verification using ONLY the evidence sentences provided.
...
# --------------------------------------------------------------------------- CRAG
CRAG_EVALUATOR = """You are a retrieval evaluator for scientific claim verification.
...
QUERY_REFORMULATION = """Rewrite the following scientific claim as a search query for a corpus of biomedical abstracts.
...
# --------------------------------------------------------------------------- Agentic RAG
AGENT_CONTROLLER = """You are the controller of an iterative retrieval agent that gathers evidence to verify a scientific claim.
...
# --------------------------------------------------------------------------- Graph construction
ENTITY_RELATION_EXTRACTION = """Extract a knowledge graph from the scientific abstract below.
...
LIGHTRAG_KEYWORDS = """Extract retrieval keywords from the scientific claim below at two levels.
...
COMMUNITY_SUMMARY = """Summarise the following group of related entities and relations from a corpus of scientific abstracts
...
ALL_PROMPTS = {
    "GENERATION_2CLASS": GENERATION_2CLASS,
    "GENERATION_3CLASS": GENERATION_3CLASS,
    "CRAG_EVALUATOR": CRAG_EVALUATOR,
    "QUERY_REFORMULATION": QUERY_REFORMULATION,
    "AGENT_CONTROLLER": AGENT_CONTROLLER,
    "ENTITY_RELATION_EXTRACTION": ENTITY_RELATION_EXTRACTION,
    "LIGHTRAG_KEYWORDS": LIGHTRAG_KEYWORDS,
    "COMMUNITY_SUMMARY": COMMUNITY_SUMMARY,
}


def export_prompts(path: str) -> None:
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Prompts used in the benchmark (verbatim)\n\n")
